...
-- combine purchase_log and view_log datasets but only get click that has purchase within +30d after click
clicks_before_purchase AS (
  SELECT
    ...
    cl.campaign_id,
	cl.medium,
    cl.view_date AS view_date,
    ABS(DATEDIFF('day', cl.view_date, pl.purchase_date)) AS diff_days
  FROM purchase_log AS pl
  INNER JOIN view_log AS cl ON cl.external_user_id = pl.external_user_id
  WHERE cl.view_date <= pl.purchase_date
    AND DATEDIFF('day', cl.view_date, pl.purchase_date) <= 30
),
...
arranged_click AS (
  SELECT 
    *,
    ROW_NUMBER() OVER (PARTITION BY purchase_id ORDER BY view_date DESC) AS rn  -- this is for CRM purpose. Every purchase got attributed to last click
