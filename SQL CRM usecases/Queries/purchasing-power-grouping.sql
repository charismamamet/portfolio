...
FROM (
  ...
  FROM (
    -- 🏗️ Innermost subquery: computes per-user aggregated purchase behavior metrics
    SELECT 
      x.user_id,
    ...
    -- 🔍 Left join to check if the seller is in the suspicious seller list
    LEFT JOIN mock_renos_db.suspicious_seller AS xs ON xs.seller_name = x.seller_name

    -- 🔍 Left join to check if the buyer is in the suspicious buyer list
    LEFT JOIN mock_renos_db.suspicious_buyer AS xb ON xb.user_id = x.user_id

    -- 📦 Filters: we want only users who bought before 21 July 2025 and never again after that
    WHERE 1
      AND x.user_id IN (
        SELECT o.user_id
